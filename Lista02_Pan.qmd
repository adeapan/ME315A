---
title: "Lab02"
author: "Artur Pan245001"
format: pdf
editor: source
---

## Conexão R >>> SQLite

```{r}
library(DBI)
library(RSQLite)

con <- dbConnect(SQLite(), "me315.sqlite")
```

### ORDER BY

1) Selecione as colunas STATE, PNO e PNAME e exiba todas as linhas, ordenadas pelos valores de STATE em ordem crescente.

##SQLite

```{sql connection=con}
SELECT * FROM PRESERVE

```
### ORDER BY 
```{sql connection=con}
SELECT STATE, PNO, PNAME FROM PRESERVE
ORDER BY STATE
```
2) elecione as colunas STATE, PNO e PNAME e exiba todas as linhas, ordenadas pelos valores de STATE em ordem decrescente.

```{sql connection=con}
SELECT STATE, PNO, PNAME FROM PRESERVE
ORDER BY STATE DESC
```
3) Selecione as colunas STATE, PNO e PNAME e exiba todas as linhas, ordenadas pelos valores de STATE e PNO, ambos em forma decrescente
```{sql connection=con}
SELECT STATE, PNO, PNAME FROM PRESERVE 
ORDER BY STATE, PNO DESC
```
4) Selecione as colunas STATE, PNO e PNAME e exiba todas as linhas, ordenadas em ordem decrescente pelos valores de PNO, nas quais STATE e AZ.

```{sql connection=con}
SELECT STATE, PNO, PNAME FROM PRESERVE
WHERE STATE = "AZ"
ORDER BY PNO DESC
```
5) Exiba o número da reserva (PNO), a área em acres (ACRES) e o nome de cada reserva natural (PNAME) localizada no estado de Arizona. Ordene o resultado pela terceira coluna.
```{sql connection=con}
SELECT PNO, ACRES, PNAME FROM PRESERVE
WHERE STATE = 'AZ'
ORDER BY 3
```
6) Exiba a tabela PRESERVE inteira, ordenada pela quarta coluna em ordem descendente e a segunda coluna em ordem crescente.

```{sql connection=con}
SELECT * FROM PRESERVE
ORDER BY 4 DESC, 2
```
7) Exiba os valores de PNO e PNAME de todas as reservas. Apresente o resultado ordenado por ACRES, em ordem decrescente.

```{sql connection=con}
SELECT PNO, PNAME FROM PRESERVE
ORDER BY ACRES DESC
```

8) Exiba os valores de STATE, FEE e PNAME de todas as linhas da tabela PRESERVE, em que:
STATE é a 1ª coluna de ordenação (ordem crescente);
FEE é a 2ª coluna de ordenação (ordem decrescente);
PNAME é a 3ª coluna de ordenação (ordem decrescente).

```{sql connection=con}
SELECT STATE, FEE, PNAME FROM PRESERVE
ORDER BY STATE, FEE DESC, PNAME DESC
```
9) Exiba os valores de PNO e ACRES das três primeiras linhas (ordenadas pelos valores do PNO).

```{sql connection=con}
SELECT PNO, ACRES FROM PRESERVE 
ORDER BY PNO
LIMIT 3
```
```{r}
Sys.setenv(TZ = "America/Sao_Paulo")
Sys.time()
```
###HANDS ON

1) Exiba a tabela inteira. Ordene o resultado pela coluna ACRES em ordem crescente.

```{sql connection=con}
SELECT * FROM PRESERVE
ORDER BY ACRES
```
2) Exiba o nome da reserva (PNAME) e a taxa de entrada (FEE) de cada reserva natural localizada em Montana.

```{sql connection=con}
SELECT PNAME, FEE FROM PRESERVE
WHERE STATE = 'MT'
ORDER BY PNAME DESC
```
4) Exiba as colunas FEE e ACRES de todas as linhas da tabela. Ordene as linhas exibidas por ACRES dentro de FEE (FEE é o campo de ordenação principal, e ACRES é o campo de ordenação secundário.)

```{sql connection=con}
SELECT FEE, ACRES FROM PRESERVE
ORDER BY FEE, ACRES
```
4) Suponha (irrealisticamente) que os valores de PNO sejam considerados confidenciais. Exiba o valor de PNAME para cada reserva natural localizada no Arizona. Apresente o resultado em ordem crescente de PNO, sem exibir os valores de PNO.

```{sql connection=con}
SELECT PNAME FROM PRESERVE
WHERE STATE = 'AZ'
ORDER BY PNO
```
5) Exiba os valores de STATE, FEE e PNO para todas as reservas naturais que tenham mais de 100 acres. Ordene a tabela resultante da seguinte forma:
STATE é o campo de ordenação de primeiro nível, em ordem decrescente;
FEE é o campo de ordenação de segundo nível, em ordem decrescente;
PNO é o campo de ordenação de terceiro nível, em ordem crescente.

```{sql connection=con}
SELECT STATE, FEE, PNO FROM PRESERVE
ORDER BY STATE DESC, FEE DESC, PNO
```
6) Exiba todas as linhas em que o valor de STATE seja maior ou igual à letra M.

```{sql connection=con}
SELECT * FROM PRESERVE 
WHERE STATE >= 'M'
```
7) Exiba todas as linhas em que o valor do nome da reserva (PNAME) seja menor que TATKON.
```{sql connection=con}
SELECT * FROM PRESERVE
WHERE PNAME < 'TATKON'
```
Os exercícios a seguir fazem referência à tabela EMPLOYEE.

8) Exiba a tabela EMPLOYEE inteira, ordenada pelo nome do funcionário em ordem crescente.
```{sql connection=con}
SELECT * FROM EMPLOYEE
ORDER BY ENAME
```
9) Exiba o nome e o salário de qualquer funcionário cujo salário seja superior a $ 2.000,00. Ordene o resultado pelo salário em ordem decrescente.

```{sql connection=con}
SELECT ENAME, SALARY FROM EMPLOYEE
WHERE SALARY > 2000
ORDER BY SALARY
```
10) Exiba o número do departamento (DNO), o número do funcionário (ENO) e o nome do funcionário de todos os funcionários (ENAME). Ordene o resultado pelo número do funcionário (em ordem crescente) dentro do número do departamento (em ordem decrescente).
```{sql connection=con}
SELECT DNO, ENO, ENAME FROM EMPLOYEE
ORDER BY ENO, DNO DESC
```

