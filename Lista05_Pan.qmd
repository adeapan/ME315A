...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES * 1,2 AS INCREASEDAREA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (FEE + 50.0) / 2.0 AS NEWFEE FROM PRESERVE
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT FEE) AS NDETAXAS FROM PRESERVE
WHERE FEE > 0
```
...
```{sql connection=con}
SELECT (COUNT(PNAME) - COUNT(DISTINCT PNAME)) AS RESERVASREPETIDAS FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS MEDIA_TAXA_NOVA FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT SUM(SALARY) AS TOTAL, AVG(SALARY) AS MEDIA, MIN(SALARY) AS MINIMO, MAX(SALARY) AS MAXIMO FROM EMPLOYEE 
```

#12
Quantos funcionários trabalham no Departamento 20?
```{sql connection=con}
SELECT COUNT(DNO) AS N_DPTO_20 FROM EMPLOYEE
WHERE DNO = 20
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS N_DE_DPTOS FROM EMPLOYEE
```
```{r}
